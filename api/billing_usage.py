...
async def get_anon_total_used(supabase: Any, sb_execute: Callable, anon_id: str) -> int:
    ...
    try:
        ...
        # Backward-compatible fallback when anon_usage table is not provisioned.
        try:
            syn = (_synthetic_email_from_anon_id(aid) or "").strip().lower()
            if syn:
                r = await _exec_sb(
                ...
                return max(0, int(getattr(r, "count", 0) or 0))
        except Exception:
            pass
    return 0
...
async def increment_anon_usage(supabase: Any, sb_execute: Callable, anon_id: str) -> None:
    ...
    try:
        ...
        # If anon_usage table is absent, fallback counting via llm_call_logs still enforces limits.
        print(f"[anon_usage] increment skipped: {e}")
...
async def fetch_active_license_row(
    ...
    # Team/member keys may exist outside `licenses` (created by team endpoints).
    if not license_row and req_key:
        try:
            tr = await _exec_sb(
            ...
            trow = getattr(tr, "data", None) if tr is not None else None
            if trow:
                license_row = {
                    "license_key": req_key,
                    "plan": "team",
                    "active": True,
                    "email": (trow.get("admin_email") or "").strip().lower() or None,
                    "user_id": None,
                    "team_id": trow.get("id"),
                }
                brand_voice = trow.get("brand_voice")
        except Exception:
            pass
    ...
    if not license_row and req_key:
        try:
            mr = await _exec_sb(
                ...
                .select("team_id,email,teams(id,active,brand_voice)")
                .eq("member_key", req_key)
                .maybe_single(),
                sb_execute,
            )
            mrow = getattr(mr, "data", None) if mr is not None else None
            team_obj = (mrow or {}).get("teams") if isinstance(mrow, dict) else None
            if mrow and team_obj and team_obj.get("active"):
                license_row = {
                    "license_key": req_key,
                    "plan": "team",
                    "active": True,
                    "email": (mrow.get("email") or "").strip().lower() or None,
                    "user_id": None,
                    "team_id": team_obj.get("id"),
                }
                brand_voice = team_obj.get("brand_voice")
        except Exception:
            pass
...
async def check_rate_limit_impl(
    *,
    ...
    # ── Anonymous trial (no JWT, no license): anon_id lifetime cap.
    # If anon_id is absent but email is present, continue into free-email path below.
    if not user_id and not has_license:
        aid = (req_anon or "").strip()
        if not aid and not req_email:
            raise HTTPException(
                status_code=400,
                detail=(
    ...
    if not resolved_uid and license_row and license_row.get("email"):
        ...
        if em:
            try:
                ...
                resolved_uid = None
    if not resolved_uid and license_key:
        try:
            resolved_uid = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"replypals:key:{license_key}"))
        except Exception:
            resolved_uid = None
